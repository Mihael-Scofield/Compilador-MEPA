...
%token MAIOR_QUE READ WRITE NOT BOLLEANO T_TRUE T_FALSE DO
...
/*23. Tomaz */
comando_repetitivo:
   WHILE expressao DO comando_composto_while
;
...
expressao_simples: /*[+|-]<termo>{(+|-|or) <termo>}*/
   ...
   termos {resolveTipoTermoFatores(&pilha_T,&pilha_T);}
   |
   termo termos {resolveTipoTermoFatores(&pilha_T,&pilha_T);}
   |
   MAIS termo
   |
   MENOS
...
/*28. Tomaz*/
termo:
   fator MULTIPLICA fator 
   { 
      geraCodigo (NULL, "MULT");
      resolveTipoTermoFatores(&pilha_T,&pilha_F);
   }
   |fator DIVIDE fator 
   { 
   ...
   | fator { pilha_vs_type_push(&pilha_T, pilha_vs_type_pop(&pilha_F));}
;
