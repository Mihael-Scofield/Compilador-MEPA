...
lista_idents_dec_var: 
   ...
   {
      tabela_simbolos_push(&ts, token, var_simples, nv_lex, num_vars, desc_type, NULL);
      num_vars++;
   }
   | IDENT 
   {
...
declaracao_de_procedimento: 
   ...
   IDENT {
      tabela_simbolos_push(&ts, token, proc_cs, nv_lex, -1, desc_type, p_rotulos.pilha[p_rotulos.topo]);
   }
...
atribuicao_ou_chamda_de_procedimento:
   ...
   | 
   {
      geraCodigo2ParamsCharNum(NULL, "CHPR ", ts.pilha[l_elem].rotulo, nv_lex);
...
/*24. Tomaz */
lista_de_expressoes:
   lista_de_expressoes VIRGULA expressao FECHA_PARENTESES
   | expressao
   | 
;
...
//       // tabela_simbolos_push(&ts, token,  categoria, nv_lex, num_vars, NULL);
//       // num_vars++;
//    }
// ;
