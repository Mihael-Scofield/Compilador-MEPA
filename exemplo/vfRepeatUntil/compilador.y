...
%token FUNCTION IF THEN ELSE GOTO WHILE REPEAT UNTIL T_END ATRIBUICAO MAIS MENOS
...
comandos_repeat_until:
   comandos_repeat_until comando PONTO_E_VIRGULA
   | comando PONTO_E_VIRGULA
;
...
/*23.b Tomaz - Modificação Avaliação*/
comando_repeat_until:
// https://www.tutorialspoint.com/pascal/pascal_repeat_until_loop.htm
// Segundo a pagina tem que fazer pelo menos uma vez.
   REPEAT
   {
      geraRotulo(&p_rotulos, &n_rotulos);
      geraCodigo(p_rotulos.pilha[p_rotulos.topo], "NADA");//R00:NADA
   }
   comandos_repeat_until
   UNTIL
   expressao 
   {
      verifica_ultimo_termo_booleano(&pilha_T);
      pilhas_vs_type_reset(&pilha_F, &pilha_T);
      geraCodigoParams(NULL, "DSVF ", p_rotulos.pilha[p_rotulos.topo]); //DSVF R00
      pilha_string_pop_n(&p_rotulos,1); 
   }
;
