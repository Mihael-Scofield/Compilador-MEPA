...
int tabela_simbolos_remove_labels_do_nv_lex(tab_simbolos* ts, int nv_lex){
   ...
   for(int i=ts->topo; i>-1; i--){
      t_simbolo* simb = &ts->pilha[i];
      if (simb->categoria == label_cs && simb->nv_lex ==nv_lex){
         ts->topo--;
         n_pfs++;
      }
      else{
   ...
   #ifdef DEBUG
      printf ("tabela_simbolos_remove_labels_do_nv_lex:.\n");
      print_tabela_simbolos(ts);
   #endif
   return n_pfs;
}
...
int tabela_simbolos_busca_simbolo_sr(tab_simbolos* ts, int nv_lex){
   #ifdef DEBUG
      printf ("tabela_simbolos_busca_simbolo_sr:\n    Procurando Simbolo SR de nv_lex...\n");
   #endif
   for(int i=ts->topo; i>-1; i--){
      t_simbolo* simb = &ts->pilha[i];
      #ifdef DEBUG
         printf ("      Comparando %s nv_lex %d com nv_lex %d .\n", simb->identificador, simb->nv_lex, nv_lex);
      #endif
      if (simb->categoria==proc_cs ||  simb->categoria==funcao_cs){
         if(simb->nv_lex==nv_lex){
            #ifdef DEBUG
               printf ("      Simbolo identificado. \"%s\"\n",simb->identificador);
               print_simbolo(simb);
            #endif
   ...
   #ifdef DEBUG
      printf ("      Sem Simbolo identificado.\n");
   #endif
   return 0;
}
...
void chamada_desvio_goto(tab_simbolos* ts, char* token, int nv_lex){
   int indx_label = tabela_simbolos_busca_simbolo(ts, token);
   t_simbolo* simb_label = &ts->pilha[indx_label];
   if(simb_label->categoria!=label_cs){
      fprintf(stderr, "Erro Semântico (%d,%d):\n   Em chamada de desvio.\n", nl, nc);
      fprintf(stderr, "   Desvio deveria chamar label. \"%s\" não é um label. \n", simb_label->identificador);
      exit(-1);
   }
   geraCodigo3ParamsCharNumNum(NULL, "DSVR ", simb_label->rotulo, simb_label->nv_lex, nv_lex);
   return;
}
...
void chamada_target_goto(tab_simbolos* ts, char* token, int nv_lex){
   ...
   int indx_sr = tabela_simbolos_busca_simbolo_sr(ts, nv_lex);
   int n;
   if (indx_sr==0){
      n=0;
   }
   else{
      n=ts->pilha[indx_sr].n_params;
   }
   geraCodigo2ParamsNum(simb_label->rotulo, "ENRT ", nv_lex, n);
   return;
}