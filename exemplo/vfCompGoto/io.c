...
void print_simbolo(t_simbolo* simb){
   ...
   switch (simb->categoria){
      ...
      case proc_cs:
         ...
         break;
      case label_cs:
         printf("CAT [LABL], nv_lex %d, rótulo %s", simb->nv_lex, simb->rotulo);
         break;
      default:
         break;;
   }
...
void print_pilha_inf_func(pilha_inf_f* pi){
   printf("Pilha INF Funcoes:\n");
   for (int i=0; i<pi->topo+1; i++){
      printf("    [%d] - %d vars(%d)\n", i, pi->pilha[i].indx_fun, pi->pilha[i].num_p);
   }
   return;
}
