...
void chamada_desvio_funcao(tab_simbolos*  ts, pilha_int* p_Funcoes, int num_vars, int nv_lex, pilha_vs_type* pilha_F, pilha_vs_type* pilha_T){
   if(p_Funcoes->topo<0){
      fprintf(stderr, "Erro do Compilador (%d,%d):\n  chamada_desvio_funcao:\n      Não tem função para chamada de desvio.\n", nl, nc);
      exit(-1);
   }

   t_simbolo* simb = &ts->pilha[p_Funcoes->pilha[p_Funcoes->topo]];
...
void carrega_identidade_ou_aloca_mem_para_funcao(tab_simbolos* ts, char* token, int  l_elem, int indx_param, pilha_vs_type* p_fator, pilha_int* p_funcoes){

   int indx_var = tabela_simbolos_busca_simbolo(ts, token);
   if(p_funcoes->topo!=-1){
      carrega_identidade_em_funcao(ts, indx_var, l_elem, indx_param, p_fator, p_funcoes);
      return;
   }
   ...
   if(l_elem<0 ){
      ...
      carrega_identidade_em_atribuicao(param_real, p_fator, l_elem, p_funcoes, indx_var);
      return;
   }
   ...
   if(simb_l_elem->categoria==proc_cs){
      ...
      carrega_identidade_em_procedimento(ts, simb_l_elem, indx_var, indx_param,  p_fator, p_funcoes);
   }
   return;
}

void carrega_identidade_em_procedimento(tab_simbolos* ts, t_simbolo* simb_proc,  int indx_var, int indx_param, pilha_vs_type* p_fator, pilha_int* p_funcoes){
   // Quando For procedimento
   if(indx_param>(simb_proc->n_params-1)){
      fprintf(stderr, "Erro do Compilador (%d,%d):\n carrega_identidade_valor_endereco_vi: procedimento \"%s\" não tem %d parametros.\n", nl, nc, simb_proc->identificador, indx_param+1);
      exit(-1);
   }
   t_simbolo* param_real = &ts->pilha[indx_var];
   t_pinf* param_formal = &simb_proc->param_infs[indx_param];
   carrega_identidade_em_sub_rotina(simb_proc, param_real, param_formal, p_fator, indx_param);
   return;
}

void carrega_identidade_em_funcao(tab_simbolos* ts, int indx_var, int  l_elem, int indx_param, pilha_vs_type* p_fator, pilha_int* p_funcoes){
   printf("carrega_identidade_em_funcao:\n");
   t_simbolo* simb_func = &ts->pilha[p_funcoes->pilha[p_funcoes->topo]];
   printf(" Pilha de Funcões:\n");
   print_pilha_int(p_funcoes);
   ...
   t_simbolo* param_real = &ts->pilha[indx_var];
   t_pinf* param_formal = &simb_func->param_infs[indx_param];
   carrega_identidade_em_sub_rotina(simb_func, param_real, param_formal, p_fator, indx_param);
   return;
}

void carrega_identidade_em_sub_rotina(t_simbolo* simb_subrotina, t_simbolo* p_real, t_pinf* p_formal, pilha_vs_type* pilha_fator, int indx_param){
...
void carrega_identidade_em_atribuicao(t_simbolo* p_real, pilha_vs_type* p_fator, int l_elem, pilha_int* p_funcoes, int indx_param){
   ...
   if(p_real->categoria==funcao_cs && l_elem!=DESCONHECIDO){
      geraCodigo1ParamNum(NULL, "AMEM ", 1);
      pilha_int_push(p_funcoes ,indx_param);
      print_pilha_int(p_funcoes);
      return;
   }
