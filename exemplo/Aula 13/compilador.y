...
declaracao_de_procedimento: 
   ...
   bloco 
   {
      ...
      geraCodigo2ParamsNum(NULL,"RTPR ", nv_lex, num_vars);
...
atribuicao:
   ATRIBUICAO
   {
      ...
      pilha_inteiros_reset(&p_Funcoes);     
   }
   expressao
   {
...
chamada_subrotina_func_args:
   ABRE_PARENTESES
   {
      //Vai dar ruin com funcoes aninhadas
      num_vars=0;
   } 
   lista_de_expressoes_sr 
   FECHA_PARENTESES
   {
      ...
      chamada_desvio_funcao(&ts, &p_Funcoes, num_vars, nv_lex, &pilha_F, &pilha_T);
      pilha_vs_type_push(&pilha_F, ts.pilha[p_Funcoes.pilha[p_Funcoes.topo]].tipo);
      pilha_int_pop(&p_Funcoes);
   }
   |
;
